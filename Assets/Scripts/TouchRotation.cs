...
public class TouchRotation : MonoBehaviour {
    ...
    RectTransform wheelTrasnform;
    void Start()
    {
        wheelTrasnform = gameObject.GetComponent<RectTransform>();
        pointerY = Input.GetAxis("Mouse Y");
    }

    public void canDrag(bool draggable)
    {
        print(isDraggable);
        if (isDraggable==true)
        {
        ...
        if (isDraggable)
        {
            ...
            else if (Input.touchCount > 0)
            {
                ...
                if (touch.position.x > Screen.width / 2)
                {
                    ...
                    {
                        i_direction = 1;
                        wheelTrasnform.Rotate(Vector3.forward, f_difX * Time.deltaTime * force);
            ...
            else
            {
                ...
                if (f_difX < 0.5f) f_difX += 1f;
                wheelTrasnform.Rotate(Vector3.forward, f_difX * i_direction * Time.smoothDeltaTime);
